...
### ✅ 完成
- 实现每日自动备份到手机文件功能：
  - **自动检测**：每次打开 App 自动检查，距离上次备份超过 1 天自动触发下载
  - **备份文件**：JSON 格式，文件名包含日期，直接下载到手机"下载"文件夹
  - **设置开关**：在「我的」页面可开启/关闭每日自动备份（默认开启）
  - **上次备份时间**：设置页显示"上次备份：今天/昨天/N天前/从未备份"
  - **首页提醒横幅**：
    - 从未备份 → 黄色警告 +「立即备份」按钮
    - 超过3天未备份 → 黄色警告 +「立即备份」按钮
    - 1-2天 → 绿色轻提醒 +「再次备份」按钮
    - 今天已备份 → 绿色"今日已自动备份 💾"
- 修改文件：
  - `js/storage.js` — 新增 `shouldAutoBackup()`, `getLastBackupText()`, `setAutoBackupEnabled()`
  - `js/app.js` — 新增 `triggerBackupDownload()`, `performAutoBackup()`, 升级备份提醒和设置页
  - `index.html` — 设置页新增自动备份开关（Toggle Switch）+ 上次备份时间
  - `css/style.css` — 新增 `.toggle-switch` 开关组件样式

### 自动备份流程
```
用户打开 App → 检查今天是否已备份
  ├─ 今天已备份 → 不重复备份，首页显示绿色"今日已自动备份"
  └─ 今天未备份 + 开关开启 → 0.8秒后自动下载备份文件
       ├─ 成功 → Toast "📦 每日自动备份完成！文件已保存到手机"
       └─ 浏览器阻止 → 静默失败，首页提醒横幅引导手动备份
```

---

## 2026-07-24 — AI提示词升级 + 成语字段扩展

### ✅ 完成
- AI 提示词改写：从"报考+选岗+备考"转向专注**公考全方面知识学习**
- 成语字段扩展（3→8）：
  - 新增：词性、用法偏向（真题中实际用法）、近义词、反义词、拓展（典故/文化常识）
  - 意思（原"释义"改名）
- 对应更新：表单/搜索/复习展示/AI解析
...
### ✅ 完成
- 新增第5个积累模块：**固定搭配** (`collocations`)
- 字段：固定搭配、释义、用法示例、考查要点
- 全面集成：
  - 存储层（`gka_collocations` / ID / 去重 / 导出导入）
  - UI层（表单/卡片/搜索/模块选择弹窗）
  - 复习模块（SM-2 间隔重复）
  - AI模块（提示词模板 + 智能解析 + 存入）
  - 首页统计 + 子标签切换
...
### 📋 待办
- [ ] 部署到 GitHub Pages 后在手机上实测自动下载
- [ ] 第 7 步：联调测试完善
- [ ] 第 8 步：部署上线

### 🔧 下一步
在手机上测试自动备份流程，验证下载文件是否正常保存
